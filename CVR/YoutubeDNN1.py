# 这里sku可以想象成用户搜索的词语
# __label__1605113209 881589453
# __label__1605113209代表的是用户ID对应的标签，即用户看的视屏的ID
# 881589453 代表的是用户搜索的token的ID
#模型学习的Embedding向量是什么？代码中的nce_weights就是视屏的Embedding向量。输入是token的Embedding
# 输入用户搜索过的token，学习到用户向量，用户向量与视屏向量做内积并计算softmax，sample负label，计算交叉熵（相应细节封装在nce_loss）
#这里需要注意：nce_loss内部只采样了部分负label进行计算loss，而后面计算出的out_layer包含了所有的label。这里需要注意一下。
