...
from CalculatePRF.MacroWithPython import *
import yaml
...
tf.app.flags.DEFINE_integer("valid_step",5,'训练多少步后执行一次验证并保存模型')
tf.app.flags.DEFINE_integer("valid_num_batch",2,'执行valid时跑多少个batch的数据')
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        try:
            ...
            while not coord.should_stop():#这里是永远不会停止的，因为epoch设置的是NOne
                ...
                train_fact_temp=([bytes.decode(e) for e in train_fact_v])
